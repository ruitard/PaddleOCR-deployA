...
void PermuteBatch::Run(const std::vector<cv::Mat> imgs, float *data) {
    for (int j = 0; j < imgs.size(); j++) {
        ...
        for (int i = 0; i < rc; ++i) {
            cv::extractChannel(imgs[j], cv::Mat(rh, rw, CV_32FC1, data + (j * rc + i) * rh * rw),
                               i);
        }
    }
}
...
void CrnnResizeImg::Run(const cv::Mat &img, cv::Mat &resize_img, float wh_ratio, bool use_tensorrt,
    ...
    cv::copyMakeBorder(resize_img, resize_img, 0, 0, 0, int(imgW - resize_img.cols),
                       cv::BORDER_CONSTANT, {127, 127, 127});
}

void ClsResizeImg::Run(const cv::Mat &img, cv::Mat &resize_img, bool use_tensorrt,
    ...
    cv::resize(img, resize_img, cv::Size(resize_w, imgH), 0.f, 0.f, cv::INTER_LINEAR);
    if (resize_w < imgW) {
        cv::copyMakeBorder(resize_img, resize_img, 0, 0, 0, imgW - resize_w, cv::BORDER_CONSTANT,
                           cv::Scalar(0, 0, 0));
    }
}

void TableResizeImg::Run(const cv::Mat &img, cv::Mat &resize_img, const int max_len) {
    int w = img.cols;
    int h = img.rows;
...
void TablePadImg::Run(const cv::Mat &img, cv::Mat &resize_img, const int max_len) {
    int w = img.cols;
    int h = img.rows;
    cv::copyMakeBorder(img, resize_img, 0, max_len - h, 0, max_len - w, cv::BORDER_CONSTANT,
                       cv::Scalar(0, 0, 0));
}

void Resize::Run(const cv::Mat &img, cv::Mat &resize_img, const int h, const int w) {
